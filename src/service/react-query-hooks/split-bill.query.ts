...
import { QueryKeys, SplitBillGroupQueryKeys, SplitBillQueryKeys } from "service/react-query-hooks/query-keys";
...
export function useCreateSplitBill(options: CustomMutationOptions) {
    ...
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (bill:SplitBill) => SplitBillService.getInstance().create(bill),
        onSuccess: () => {
            onSuccess && onSuccess();
